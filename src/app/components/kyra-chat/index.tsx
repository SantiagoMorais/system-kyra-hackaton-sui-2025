...
export const KyraChat = () => {
  ...
  const handleSuggestedContent = ({ content }: { content: string }) => {
        ...
        <ul className="mb-5 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kyraChatCards.map(({ text }, index) => (
            <motion.li
              ...
              onClick={() => handleSuggestedContent({ content: text })}
              className="border-grey text-secondary flex-1 cursor-pointer border px-2.5 py-4 pb-6 duration-500 hover:border-orange-600 lg:pb-10"
            >
              <button>{text}</button>
            </motion.li>
          ))}
        </ul>
        <TextArea suggestedContent={suggestedContent} />
      </article>
    </section>
  );
};
